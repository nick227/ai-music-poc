...
def write_command_manifest(
    ...
    *,
    ...
    payload = {
        ...
        "artifact_final_dir": str(adapter_final_dir(output_dir)),
    }
    path = output_dir.parent / "ace_train_command.json"
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    return path


def write_artifact_manifest(output_dir: Path, final_dir: Path) -> Path | None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    append_log(log_path, f"[ace_train_runner] output_dir={output_dir}")
    ...
    if preprocess_command and run_command(preprocess_command, cwd=ace_step_dir, env=env, log_path=log_path) != 0:
        return 1
    if train_command and run_command(train_command, cwd=ace_step_dir, env=env, log_path=log_path) != 0:
        return 2

    manifest = write_artifact_manifest(output_dir, adapter_final_dir(output_dir))
    if manifest is None:
